...
public class BTree implements Serializable  
{
    ...
    * ***********************************************************************/
    private class BNode implements Serializable
    {
        int size;
        ...
        public BNode(int t) 
        {
            this.key = new String[t-1]; //allocates max num of keys and child references
            this.data = new Object[t-1];
            this.child = new BNode[t];
            this.leaf = true; //sets node to have a leaf
        }
    }

    private BNode root;
    private int t = 3; //degrees
    private int count = 0; //variable count
    private int height = 0; //height of tree
    private int minKey, minChild, maxKey, maxChild;

    //CONSTRUCTORS
    public BTree(int inT) 
    {
        t = inT; //sets new degree
        minKey = t;
        minChild = minKey + 1;
        maxKey = 2 * minKey;
        maxChild = maxKey + 1;
        root = new BNode(maxChild);
    ...
    public BTree(String fileName, int inT) 
    {
        t = inT;//sets new degree
        root = new BNode(t);
        root.leaf = true;
    ...
    * All282project2/New%20folder/Morada/src/Tree234.java
    ...
    ***************************************************************************/
    ...
    public void insert(String key, Object data)
    {
        ...
        else
        {
            ...
            if (r.size ==  t) 
            {
                ...
                int i = 0;
                if (parent.key[0].compareTo(key)<0)
                {
                    i++;
                }
                insertNonfull(parent.child[i], key, data);//inserts values into nonfull node
    ...
    {
        int ideal_height = (int)(Math.log(count)/Math.log(maxChild));
        int actual_height = height;
        double balance = ((double)ideal_height/(double)actual_height)/100;
        return balance;
    }
}
